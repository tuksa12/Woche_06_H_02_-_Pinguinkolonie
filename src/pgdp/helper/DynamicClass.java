	...
	public Class<T> getC() {
		if (c == null) {
			try {
				...
				throw new TaskNotCompletedException("Klasse/Interface " + cName + " nicht gefunden");
			}
		}
		return c;
	}
		...
		for (Method m : getC().getDeclaredMethods()) {
			if (Modifier.isPublic(m.getModifiers())) {
				...
				if (objectMethods.contains(sig))
					continue;
				throw new TaskNotCompletedException("Methode " + sig + " ist public, sollte sie aber nicht");
			}
		}
	}
	...
	public void checkForNonPrivateFields() {
		for (Field f : getC().getDeclaredFields()) {
			if(!Modifier.isPrivate(f.getModifiers()))
				throw new TaskNotCompletedException("Attribut " + f + " muss private sein.");
		}
	}
	...
	public void checkForNonFinalFields() {
		for (Field f : getC().getDeclaredFields()) {
			if(!Modifier.isFinal(f.getModifiers()))
				throw new TaskNotCompletedException("Attribut " + f + " muss final sein.");
		}
	}
}
